...
from quests.quest_calculator import QuestCalculator, WeeklyBoost
...
class QuestOptimizer:
    ...
    def rank_quests(
        ...
        results.sort(
            key=lambda x: x["pd_per_minute"] if x["pd_per_minute"] is not None else x["total_pd"],
            reverse=True
        )
    ...
    def print_rankings(
        ...
        show_section_id = len(rankings) > 0 and any(
            result.get("section_id") != rankings[0].get("section_id") 
            for result in rankings
        )
        ...
        if show_section_id:
            if has_areas:
                fixed_width = 6 + 30 + 12 + max_areas_width + 8 + 12 + 12 + 10 + 15 + reward_column_width + divider_width  # Rank + Quest Name + Section ID + Areas + Episode + PD + PD/min + Enemies + Raw PD/Quest + Quest Reward + Divider
            else:
                fixed_width = 6 + 30 + 12 + 8 + 12 + 12 + 10 + 15 + reward_column_width + divider_width  # Rank + Quest Name + Section ID + Episode + PD + PD/min + Enemies + Raw PD/Quest + Quest Reward + Divider
        else:
            if has_areas:
                fixed_width = 6 + 30 + max_areas_width + 8 + 12 + 12 + 10 + 15 + reward_column_width + divider_width  # Rank + Quest Name + Areas + Episode + PD + PD/min + Enemies + Raw PD/Quest + Quest Reward + Divider
            else:
                fixed_width = 6 + 30 + 8 + 12 + 12 + 10 + 15 + reward_column_width + divider_width  # Rank + Quest Name + Episode + PD + PD/min + Enemies + Raw PD/Quest + Quest Reward + Divider
        ...
        for idx, result in enumerate(rankings, 1):
            ...
            if show_details and result.get("enemy_breakdown"):
                ...
                section_id_display = result.get("section_id", "Unknown")
                ...
                for enemy, data in result["enemy_breakdown"].items():
                    if "error" in data:
                        ...
                        print(f"  {enemy_display:<20} {item_display:<30} {adjusted_dar:<10.6f} {adjusted_rdr:<12.8f} {actual_rate:<12.8f} {count:<8} {expected_drops:<12.8f} {item_price_pd:<12.8f} {exp_value:<12.8f}")
                print()
...
def parse_weekly_boost(value: str) -> Optional[WeeklyBoost]:
    """Parse weekly boost string to enum."""
    value_upper = value.upper()
    if value_upper == "DAR":
        return WeeklyBoost.DAR
    elif value_upper == "RDR":
        return WeeklyBoost.RDR
    elif value_upper == "RAREENEMY" or value_upper == "RARE_ENEMY":
        return WeeklyBoost.RareEnemy
    elif value_upper == "XP" or value_upper == "EXP":
        return WeeklyBoost.XP
    elif value_upper == "NONE" or value_upper == "":
        return None
    else:
        raise argparse.ArgumentTypeError(f"Invalid weekly boost: {value}. Must be DAR, RDR, RareEnemy, XP, or None")
...
Examples:
  ...
  python quest_optimizer.py --section-id All --episode 1 --top-n 20
    ...
    parser.add_argument(
        ...
        type=str,
        default="All",
        choices=["Viridia", "Greenill", "Skyly", "Bluefull", "Purplenum", 
                 "Pinkal", "Redria", "Oran", "Yellowboze", "Whitill", "All"],
    ...
    parser.add_argument(
        "--rbr",
        action="store_true",
        help="Enable RBR boost (+25%% DAR, +25%% RDR)"
    )
    
    parser.add_argument(
        "--weekly-boost",
        type=parse_weekly_boost,
        default=None,
    ...
    parser.add_argument(
        ...
        choices=[1, 2, 4],
        default=None,
        help="Filter by episode (1, 2, or 4). Omit for all episodes"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Show only top N quests (default: show all)"
    )
    
    parser.add_argument(
        "--details",
        action="store_true",
        help="Show detailed enemy breakdown and PD drop tables"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=5,
        help="Number of notable item columns to show (default: 5)"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to price guide directory (default: ../price_guide/data)"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to quests.json file (default: quests/quests.json)"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Filter to a specific quest by exact match on quest_name (shortname)"
    )
    ...
    quests_file = Path(args.quests_data) if args.quests_data else base_path / "quests" / "quests.json"
    ...
    if args.quest:
        ...
        for quest in quests_data:
            quest_name_lower = quest.get("quest_name", "").lower()
            # Exact match on quest_name (shortname)
            if quest_filter == quest_name_lower:
                filtered_quests.append(quest)
        
        if not filtered_quests:
            print(f"Error: No quest found matching '{args.quest}'")
            print("Available quests:")
            for quest in quests_data[:10]:  # Show first 10 as examples
                print(f"  - {quest.get('quest_name')} ({quest.get('long_name', 'N/A')})")
            if len(quests_data) > 10:
                print(f"  ... and {len(quests_data) - 10} more")
            return 1
        
        quests_data = filtered_quests
